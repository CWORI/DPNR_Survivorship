...
This chunk summarizes cumulative outplant survivorship from the baseline month. Each plot uses the first month with present outplants as the baseline for each monitored subsection, then combines the monitored subsections into one plot-level result. If one subsection has not been monitored through a later month, that later month only uses the subsections with data instead of counting the unmonitored subsection as dead.
:::
